...
import druidsurv.cards.monkeys.magick.GreenMage;
import druidsurv.cards.nemesis.*;
import druidsurv.relics.TodoItemNew;
import druidsurv.relics.decks.MoxDeck;
...
public class NemCharacterFile extends CustomPlayer {

    static final String ID = makeID("NemDruid");
    static final CharacterStrings characterStrings = CardCrawlGame.languagePack.getCharacterString(ID);
    static final String[] NAMES = characterStrings.NAMES;
    static final String[] TEXT = characterStrings.TEXT;


    public NemCharacterFile(String name, PlayerClass setClass) {
    ...
    public CharSelectInfo getLoadout() {
        return new CharSelectInfo(NAMES[0], TEXT[0],
                68, 68, 4, 75, 5, this, getStartingRelics(),
    ...
    public ArrayList<String> getStartingDeck() {
        ...
        for (int i = 0; i < 4; i++) {
            retVal.add(Strike_Mage.ID);
        }
        for (int i = 0; i < 1; i++) {
            retVal.add(Bash_Mage.ID);
        }
        for (int i = 0; i < 1; i++) {
            retVal.add(GreenMage.ID);
        }
        for (int i = 0; i < 2; i++) {
            retVal.add(Defend_Mage.ID);
        }
        for (int i = 0; i < 3; i++) {
            retVal.add(EmptyVessel.ID);
        }
        for (int i = 0; i < 1; i++) {
            retVal.add(MonkeySense.ID);
        }
        return retVal;
    }
    ...
    public AbstractPlayer newInstance() {
        return new NemCharacterFile(name, chosenClass);
    }
